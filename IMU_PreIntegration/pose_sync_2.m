...
Keys = [];
T = cell(7,1);
T{1} = [R_gt(:,:,1),mocap.position(:,1);[0 0 0 1]];
for i = 2:length(T)
    T{i} = [R_gt(:,:,i),mocap.position(:,i);[0 0 0 1]];
    Keys{i-1} = [i-1, i];
end
...
Sigma_g = 0.0035*2*pi/360*sqrt(2000);
Sigma_a = 0.14e-3*9.80665*sqrt(2000);
Sigma = diag([Sigma_g, Sigma_g, Sigma_g, Sigma_a, Sigma_a, Sigma_a]);
