...
void Frame::ComputeIMUPreIntSinceLastFrame(const Frame* pLastF, IMUPreintegrator& IMUPreInt) const
{
    ...
    // remember to consider the gap between the last KF and the first IMU
    {
        ...
        double dt = imu._timestamp - pLastF->mTimeStamp;
        ...
        if(dt < 0)
        {
            cerr<<std::fixed<<std::setprecision(3)<<"dt = "<<dt<<", this frame vs last imu time: "
                <<pLastF->mTimeStamp<<" vs "<<imu._timestamp<<endl;
    ...
    for(size_t i=0; i<vIMUSinceLastFrame.size(); i++)
    {
        ...
        if(i==vIMUSinceLastFrame.size()-1)
            ...
            nextt = vIMUSinceLastFrame[i+1]._timestamp;  // regular condition, next is imu data
...
Frame::Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp,
             ...
             cv::Mat &distCoef, const float &bf, const float &thDepth, KeyFrame* pLastKF)
     ...
     mvLevelSigma2(frame.mvLevelSigma2), mvInvLevelSigma2(frame.mvInvLevelSigma2)
{
    ...
    mMarginalCovInv = frame.mMarginalCovInv;
