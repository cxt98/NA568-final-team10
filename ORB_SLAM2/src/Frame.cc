...
void Frame::ComputeIMUPreIntSinceLastFrame(const Frame* pLastF, IMUPreintegrator& IMUPreInt) const
{
    ...
    // remember to consider the gap between the last KF and the first IMU
    {
        ...
        double dt = imu._time - pLastF->mTimeStamp;
        IMUPreInt.update(imu._gyr - bg, imu._acc - ba, dt);

        // Test log
        if (dt < 0)
        {
            cerr << std::fixed << std::setprecision(3) << "dt = " << dt
                 << ", this frame vs last imu time: " << pLastF->mTimeStamp
                 << " vs " << imu._time << endl;
    ...
    for (size_t i = 0; i < vIMUSInceLastFrame.size(); i++)
    {
        ...
        if (i == vIMUSInceLastFrame.size() - 1)
            ...
            nextt = vIMUSInceLastFrame[i + 1]._time; // regular condition, next is imu data
        ...
        if (dt <= 0)
        {
            cerr << std::fixed << std::setprecision(3) << "frame:dt = " << dt
