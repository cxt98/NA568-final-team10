...
IMUPreintegrator Tracking::GetIMUPreIntSinceLastKF(Frame* pCurF, KeyFrame* pLastKF, const std::vector<IMUData>& vIMUSInceLastKF)
{
    ...
    // remember to consider the gap between the last KF and the first IMU
    {
        ...
        double dt = imu._time - pLastKF->mTimeStamp;
        IMUPreInt.update(imu._gyr - bg, imu._acc - ba, dt);

        // Test log
        if (dt < 0)
        {
            cerr << std::fixed << std::setprecision(3) << "dt = " << dt << ", this KF vs last imu time: " << pLastKF->mTimeStamp << " vs " << imu._time << endl;
    ...
    for (size_t i = 0; i < vIMUSInceLastKF.size(); i++)
    {
        ...
        if (i == vIMUSInceLastKF.size() - 1)
            ...
            nextt = vIMUSInceLastKF[i + 1]._time; // regular condition, next is imu data

        // delta time
        double dt = nextt - imu._time;
        ...
        if (dt <= 0)
        {
            cerr << std::fixed << std::setprecision(3) << "tracking:dt = " << dt
                 << ", this vs next time: " << imu._time << " vs " << nextt << endl;
            std::cerr.unsetf ( std::ios::showbase );   // deactivate showbase
        }
    }
...
//NOTE   GrabImageStereoVIO(imLeft,imRight,vimu,timestamp);
cv::Mat Tracking::GrabImageStereoVIO(const cv::Mat &imRectLeft, const cv::Mat &imRectRight,
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    for (size_t i = 0; i < mvIMUSinceLastKF.size(); i++)
    {
        ...
        if (imu._time < mInitialFrame.mTimeStamp)
            vimu1.push_back(imu);
        else
            vimu2.push_back(imu);
    }
