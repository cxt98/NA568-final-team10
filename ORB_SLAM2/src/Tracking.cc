...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer,
    ...
    Tracking::Tracking(pSys, pVoc, pFrameDrawer, pMapDrawer, pMap, pKFDB, strSettingPath, sensor)
{
...
IMUPreintegrator Tracking::GetIMUPreIntSinceLastKF(Frame* pCurF, KeyFrame* pLastKF, const std::vector<IMUData>& vIMUSInceLastKF)
{
    ...
    // remember to consider the gap between the last KF and the first IMU
    {
        ...
        double dt = imu._timestamp - pLastKF->mTimeStamp;
        ...
        if(dt < 0)
        {
            cerr<<std::fixed<<std::setprecision(3)<<"dt = "<<dt<<", this KF vs last imu time: "
                <<pLastKF->mTimeStamp<<" vs "<<imu._timestamp<<endl;
    ...
    for(size_t i=0; i<vIMUSInceLastKF.size(); i++)
    {
        ...
        if(i==vIMUSInceLastKF.size()-1)
            ...
            nextt = vIMUSInceLastKF[i+1]._timestamp;  // regular condition, next is imu data

        // delta time
        double dt = nextt - imu._timestamp;
        ...
        if(dt <= 0)
        {
            cerr<<std::fixed<<std::setprecision(3)<<"dt = "<<dt<<", this vs next time: "
                <<imu._timestamp<<" vs "<<nextt<<endl;
            std::cerr.unsetf ( std::ios::showbase ); // deactivate showbase
        }
    }
...
cv::Mat Tracking::GrabImageStereoVIO(const cv::Mat &imRectLeft,
                                     ...
                                     const double &timestamp)
{
    ...
    if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, vimu,
