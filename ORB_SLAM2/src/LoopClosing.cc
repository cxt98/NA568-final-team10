...
bool LoopClosing::ComputeSim3()
{
    ...
    int nTotalMatches = 0;
    for(auto currentMatchedPoint : mvpCurrentMatchedPoints)
    {
        if(currentMatchedPoint)
            nTotalMatches++;
    }
...
void LoopClosing::CorrectLoop()
{
    ...
    Optimizer::OptimizeEssentialGraph(mpMap, mpMatchedKF, mpCurrentKF, NonCorrectedSim3,
                                      CorrectedSim3, LoopConnections, mbFixScale, this);
            ...
            while(!lpKFtoCheck.empty())
            {
                ...
                for(auto sit=sChilds.begin();sit!=sChilds.end();sit++)
                {
                    ...
                    if(pChild->mnBAGlobalForKF!=nLoopKF)
                    {
                        ...
                        Vector3d Vw2 = RwbGBA*Rw1.transpose()*Vw1;
