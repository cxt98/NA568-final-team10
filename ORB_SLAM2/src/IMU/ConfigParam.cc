...
//    std::cout<<"timestamp image delay to imu: "<<_ImageDelayToIMU<<std::endl;
    ...
    {
        cv::FileNode Tcb_ = fSettings["IMU.Tcb"];
        Eigen::Matrix<double,3,3> R;
        R << Tcb_[0], Tcb_[1], Tcb_[2],
            Tcb_[4], Tcb_[5], Tcb_[6],
            Tcb_[8], Tcb_[9], Tcb_[10];
        Eigen::Quaterniond qr(R);
        R = qr.normalized().toRotationMatrix();
        Eigen::Matrix<double,3,1> t( Tcb_[3], Tcb_[7], Tcb_[11]);
        _EigTcb = Eigen::Matrix4d::Identity();
        _EigTcb.block<3,3>(0,0) = R;
        _EigTcb.block<3,1>(0,3) = t;
        _MatTcb = cv::Mat::eye(4,4,CV_32F);
        for(int i=0;i<4;i++)
            for(int j=0;j<4;j++)
                _MatTcb.at<float>(i,j) = _EigTcb(i,j);

        _EigTbc = Eigen::Matrix4d::Identity();
        _EigTbc.block<3,3>(0,0) = R.transpose();
        _EigTbc.block<3,1>(0,3) = -R.transpose()*t;
        for(int i=0;i<4;i++)
            for(int j=0;j<4;j++)
