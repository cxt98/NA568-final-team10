...
void KeyFrame::ComputePreInt()
{
    ...
    else
    {
        ...
        // remember to consider the gap between the last KF and the first IMU
        {
            ...
            double dt = imu._time - mpPrevKeyFrame->mTimeStamp;
            mIMUPreInt.update(imu._gyr - bg,imu._acc - ba,dt);

            // Test log
            if(dt < 0)
            {
                cerr<<std::fixed<<std::setprecision(3)<<"dt = "<<dt<<", this KF vs last imu time: "<<mTimeStamp<<" vs "<<imu._time<<endl;
        ...
        for(size_t i=0; i<mvIMUData.size(); i++)
        {
            ...
            if(i==mvIMUData.size()-1)
                ...
                nextt = mvIMUData[i+1]._time;  // regular condition, next is imu data
            ...
            if(dt <= 0)
            {
                cerr<<std::fixed<<std::setprecision(3)<<"keyframe:dt = "<<dt<<", this vs next time: "<<imu._time<<" vs "<<nextt<<endl;
                std::cerr.unsetf ( std::ios::showbase );                // deactivate showbase
            }
        }
    }
