...
cv::Mat System::TrackStereoVIO(const cv::Mat &imLeft, const cv::Mat &imRight,
                               const std::vector<IMUData> &vimu, const double &timestamp)
{
    if(mSensor!=STEREO)
    {
        cerr << "ERROR: you called TrackStereoVIO but input sensor was not set to STEREO." << endl;
        exit(-1);
    }
    ...
    return mpTracker->GrabImageStereoVIO(imLeft,imRight,vimu,timestamp);
}
...
void System::SaveTrajectoryTUM(const string &filename)
{
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        //
        cv::Mat Tcb = ConfigParam::GetMatT_cb();

        cv::Mat Tbw = Tcb.inv() * Tcw;
