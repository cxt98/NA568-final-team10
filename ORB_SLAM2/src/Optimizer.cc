...
void Optimizer::GlobalBundleAdjustmentNavState(Map* pMap, const cv::Mat& gw, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for (size_t i = 0; i < vpMP.size(); i++)
    {
        ...
        for (auto mit = observations.begin(); mit != observations.end(); mit++)
        {
            ...
            else
            {
                ...
                auto e = new g2o::EdgeStereoNavStatePVRPointXYZ();
    ...
    for (auto pKF : vpKFs)
    {
        if (pKF->isBad())
            continue;
        auto vNSPVR = dynamic_cast<g2o::VertexNavStatePVR*>(optimizer.vertex(2 * pKF->mnId));
        auto vNSBias = dynamic_cast<g2o::VertexNavStateBias*>(optimizer.vertex(2 * pKF->mnId + 1));
...
int Optimizer::PoseOptimization(Frame *pFrame, Frame* pLastFrame, const IMUPreintegrator& imupreint, const cv::Mat& gw, const bool& bComputeMarg)
{
    ...
    auto vNSFPVR = new g2o::VertexNavStatePVR();
    {
    ...
    auto vNSFBias = new g2o::VertexNavStateBias();
    {
    ...
    auto vNSFPVRlast = new g2o::VertexNavStatePVR();
    {
    ...
    auto vNSFBiaslast = new g2o::VertexNavStateBias();
    {
    ...
    auto eNSPrior = new g2o::EdgeNavStatePriorPVRBias();
    {
    ...
    auto eNSPVR = new g2o::EdgeNavStatePVR();
    {
    ...
    auto eNSBias = new g2o::EdgeNavStateBias();
    {
        ...
        for (int i = 0; i < Ncur; i++)
        {
            ...
            if (pMP)
            {
                ...
                if (pFrame->mvuRight[i] < 0)
                {
                    ...
                    auto e = new g2o::EdgeNavStatePVRPointXYZOnlyPose();
                ...
                else  // Stereo observation
                {
                    ...
                    auto e = new g2o::EdgeStereoNavStatePVRPointXYZOnlyPose();
    ...
    auto vNSPVR_recov = dynamic_cast<g2o::VertexNavStatePVR*>(optimizer.vertex(FramePVRId));
    const NavState& nsPVR_recov = vNSPVR_recov->estimate();
    auto vNSBias_recov = dynamic_cast<g2o::VertexNavStateBias*>(optimizer.vertex(FrameBiasId));
...
int Optimizer::PoseOptimization(Frame *pFrame, KeyFrame* pLastKF, const IMUPreintegrator& imupreint, const cv::Mat& gw, const bool& bComputeMarg)
{
    ...
    auto vNSKFPVR = new g2o::VertexNavStatePVR();
    {
    ...
    auto vNSKFBias = new g2o::VertexNavStateBias();
    {
...
//rocky add for stereo vio
        ...
        for (auto mit = observations.begin(), mend = observations.end(); mit != mend; mit++)
        {
            ...
            if (!pKFi->isBad())
            {
                ...
                else
                {
                    ...
                    auto e = new g2o::EdgeStereoNavStatePVRPointXYZ();// 这里和单目不同
    ...
    if (!vToErase.empty())
    {
        for (auto toErase : vToErase)
        {
            KeyFrame* pKFi = toErase.first;
            MapPoint* pMPi = toErase.second;
    ...
    {
        KeyFrame* pKFi = *lit;
        auto vNSPVR = static_cast<g2o::VertexNavStatePVR*>(optimizer.vertex(2 * pKFi->mnId));
        auto vNSBias = static_cast<g2o::VertexNavStateBias*>(optimizer.vertex(2 * pKFi->mnId + 1));
...
Vector3d Optimizer::OptimizeInitialGyroBias(const std::vector<Frame> &vFrames)
{
    ...
    auto vBiasg = new g2o::VertexGyrBias();
    ...
    for (size_t i = 0; i < vFrames.size(); i++)
    {
        ...
        auto eBiasg = new g2o::EdgeGyrBias();
    ...
    auto vBgEst = static_cast<g2o::VertexGyrBias*>(optimizer.vertex(0));
