...
public:
    ...
    KeyFrameInit(KeyFrame& kf):
        mTimeStamp(kf.mTimeStamp), mpPrevKeyFrame(nullptr), Twc(kf.GetPoseInverse().clone()),
    ...
    void ComputePreInt()
    {
        ...
        else
        {
            ...
            // remember to consider the gap between the last KF and the first IMU
            {
                ...
                double dt = std::max(0., imu._timestamp - mpPrevKeyFrame->mTimeStamp);
                mIMUPreInt.update(imu._gyr - bg,imu._acc ,dt);  // Acc bias not considered here
            }
            // integrate each imu
            for(size_t i=0; i<mvIMUData.size(); i++)
            {
                ...
                if(i==mvIMUData.size()-1)
                    ...
                    nextt = mvIMUData[i+1]._timestamp;  // regular condition, next is imu data

                // delta time
                double dt = std::max(0., nextt - imu._timestamp);
                // update pre-integrator
                mIMUPreInt.update(imu._gyr - bg, imu._acc, dt);
            }
        }
    }
...
bool LocalMapping::TryInitVIO()
{
    ...
    // Step 3.
    // Use gravity magnitude 9.80665 as constraint
...
void LocalMapping::AddToLocalWindow(KeyFrame* pKF)
{
    ...
    else
    {
        ...
        while(mlLocalKeyFrames.size() < mnLocalWindowSize && pKF0->GetPrevKeyFrame()!=nullptr)
        {
