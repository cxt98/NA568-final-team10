function [r_ij, J_ij] = preintegration_factor(R_i, R_j, v_i, v_j, p_i, ...
    ...
    J_dpij = [vector_to_so3(R_i'*(p_j-p_i-v_i*sdt_ij-0.5*g*sdt_ij^2)) ...
    ...
    J_dvij = [vector_to_so3(R_i'*(v_j-v_i-g*sdt_ij)) ... % dphi_i
              -R_i' ...      % dv_i
              zeros(3,3) ... % dp_i
              zeros(3,3) ... % dphi_j
              R_i' ...       % dv_j
    ...
    J_dRij = [-inv_right_jacobian_map(r_dRij)*R_j'*R_i ... % dphi_i
              zeros(3,3) ... % dv_i
              zeros(3,3) ... % dp_i
              inv_right_jacobian_map(r_dRij) ... % dphi_j
              zeros(3,3) ... % dv_j
              zeros(3,3)];   % dp_j
